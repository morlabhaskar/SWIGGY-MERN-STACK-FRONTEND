...
import { FcPrevious,FcApproval,FcNext } from "react-icons/fc";
    ...
    <div className="w-[90%] p-2 flex flex-col justify-start mx-auto mt-4">
      <div className="flex justify-between">
        ...
        <div className="flex gap-10">
          <p className="bg-orange-400 flex justify-center items-center p-2.5 rounded-full hover:bg-slate-300" onClick={()=>handleScroll("left")}><FcPrevious/></p>
          <p className="bg-orange-400 flex justify-center items-center p-2.5 rounded-full hover:bg-slate-300" onClick={()=>handleScroll("right")}><FcNext/></p>
            ...
            <div className=' p-2' key={vendor._id}>
              {vendor.firm.map((firm) => (
                <div className='min-w-[300px] max-w-[300px] rounded-xl p-2 ' key={firm._id}>
                  <Link to={`/products/${firm._id}/${firm.firmName}`}><img className="h-[220px] w-[350px] rounded-md" src={`${API_URL}/uploads/${firm.image}`} alt="" /></Link>
                  <p className="text-lg font-semibold text-slate-700">{firm.firmName}</p>
                  <p className="text-xs flex text-blue-700"><span><MdLocationPin/></span><span>{firm.area}</span></p>
                </div>
              ))}
            </div>
          </div>
