    ...
    <section class="body-font py-1 w-[90%] mx-auto">
      ...
      <div className='flex items-center mb-5'>
        ...
        <div className='flex items-center justify-start gap-20 py-4 w-5/6'>
          <button onClick={()=>filterHandler("All")} className='border min-w-[150px] max-w-[150px] p-1 border-slate-500 rounded-xl cursor-pointer text-sm transition duration-500 hover:scale-110 hover:bg-slate-300'>All</button>
          <button onClick={()=>filterHandler("South-Indian")} className='border min-w-[150px] max-w-[150px] p-1 border-slate-500 rounded-xl cursor-pointer text-sm transition duration-500 hover:scale-110 hover:bg-slate-300'>South Indian</button>
          <button onClick={()=>filterHandler("North-Indian")} className='border min-w-[150px] max-w-[150px] p-1 border-slate-500 rounded-xl cursor-pointer text-sm transition duration-500 hover:scale-110 hover:bg-slate-300'>North Indian</button>
          <button onClick={()=>filterHandler("Bakery")} className='border min-w-[150px] max-w-[150px] p-1 border-slate-500 rounded-xl cursor-pointer text-sm transition duration-500 hover:scale-110 hover:bg-slate-300'>Bekary</button>
        </div>
      </div>
          ...
          {firmData.map((apple) => {
            return apple.firm.map((item)=>{
              if(selectRegion === "All" || item.region.includes(selectRegion.toLocaleLowerCase())){
                  ...
                  return (
                    <>
                      <Link to={`/products/${item._id}/${item.firmName}`} className="lg:w-1/4 md:w-1/2 p-4 w-full transition duration-500 hover:scale-90 cursor-pointer">
